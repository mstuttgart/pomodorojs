...
const toogleTimer = () => {
  if (!isTimerRunning) {
    ...
    intervalID = setInterval(() => {
      ...
      if (secondsTimeLeft >= 0) updateTimer();
      ...
      if (secondsTimeLeft == 0) {
        ...
        if (sessionInfo.current == "pomodoro") {
          ...
          if (sessionInfo.counter == 4) showNotification("Take a long break!");
          else showNotification("Take a break!");
        } else {
...
buttonSettingsModal.onclick = () => {
  ...
  document.getElementById("btmSave").addEventListener("click", () => {
    ...
    sessionInfo.type["pomodoro"].duration =
      localStorage.getItem("pomodoroValue");
    sessionInfo.type["shortBreak"].duration =
      localStorage.getItem("shortBreakValue");
    sessionInfo.type["longBreak"].duration =
      localStorage.getItem("longBreakValue");
...
buttonAboutModal.onclick = () => {
  ...
  // Get the <span> element that closes the modal
  document
    .getElementById("btmCloseAboutModal")
    .addEventListener("click", () => {
      aboutModal.style.display = "none";
    });

  document
    .getElementById("backgroundAboutModal")
    .addEventListener("click", () => {
      aboutModal.style.display = "none";
    });
};
